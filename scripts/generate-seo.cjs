  ...
  return (project.sections || [])
    .map((section) => {
      ...
      const items = section.items
        ...
        : "";
      return `<section class="section"><h2>${escapeHtml(section.heading)}</h2>${body}${items}</section>`;
    })
    .join("\n");
}
  ...
  return layout({
    ...
    body: `
      ...
      <article>
        ...
        <p class="stack">${escapeHtml(project.stack.join(" / "))}</p>
...
function notFoundPage() {
  return layout({
    ...
    description: "The requested page could not be found on Deven Varu's portfolio.",
